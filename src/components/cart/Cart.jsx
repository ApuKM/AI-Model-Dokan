...
const Cart = ({ cartModel, cartModels, setCartModels }) => {
  const handleDelete = (cartModel) => {
    const filteredModels = cartModels.filter(
      (item) => item.id !== cartModel.id,
    );
    setCartModels([...filteredModels]);
  };
  return (
    <>
      <div className="flex items-center justify-between border border-info/10 rounded-lg p-4 shadow-lg">
        ...
        <button className="btn text-red-500 bg-red-100">
          <Trash className="w-4" onClick={() => handleDelete(cartModel)} />
        </button>
      </div>
    </>
  );
